...
class ValidateInventoryItem implements UseCase<Unit, InventoryItem> {
  ...
  Future<Either<Failure, Unit>> call(InventoryItem params) async {
    ...
    if (params.currentStock < 0) {
      return const Left(ValidationFailure('El stock actual no puede ser negativo'));
    }

    // Validar stock mínimo
    if (params.minimumStock < 0) {
      return const Left(ValidationFailure('El stock mínimo no puede ser negativo'));
    }

    // Validar stock máximo
    if (params.maximumStock <= 0) {
      return const Left(ValidationFailure('El stock máximo debe ser mayor a 0'));
    }

    // Validar que el stock máximo sea mayor al mínimo
    if (params.maximumStock <= params.minimumStock) {
      return const Left(ValidationFailure('El stock máximo debe ser mayor al stock mínimo'));
    }
    ...
    if (params.batchNumber.trim().isEmpty) {
      return const Left(ValidationFailure('El número de lote es requerido'));
    }

    // Validar que el stock actual no exceda significativamente el máximo
    if (params.currentStock > params.maximumStock * 1.5) {
      return const Left(ValidationFailure('El stock actual excede significativamente el stock máximo recomendado'));
    }

    // Validar usuario que actualizó
    if (params.lastUpdatedBy.trim().isEmpty) {
      return const Left(ValidationFailure('El usuario que actualizó es requerido'));
    }
